...
```bash
api-teste$ sam local invoke NetCodeWebAPIServerless --event events/event.json
```

The AWS SAM CLI can also emulate your application's API. Use the `sam local start-api` command to run the API locally on port 3000.

```bash
api-teste$ sam local start-api
api-teste$ curl http://localhost:3000/
```
...
```bash
api-teste$ sam logs -n NetCodeWebAPIServerless --stack-name api-teste --tail
```
...
```bash
api-teste$ dotnet test test/ServerlessAPI.Tests/ServerlessAPI.Tests.csproj 
```
...
```bash
sam delete --stack-name api-teste
```
